...
def build_knowledge_base(
    ...
    for sheet_name, cases in parsed_payload.items():
        # Skip non-sheet sections (for example: meta generated by clean_testrail_json.py).
...
def build_case_object(case: Dict[str, Any]) -> Dict[str, Any]:
    case_obj = {
        "case_id": case.get("case_id"),
    ...
    for step in case.get("steps", []):
        ...
        if "action_substeps" in step:
            step_obj["action_substeps"] = clean_text_list(step.get("action_substeps", []))
        if "expected_checkpoints" in step:
            step_obj["expected_checkpoints"] = clean_text_list(step.get("expected_checkpoints", []))
        case_obj["steps"].append(step_obj)

    return case_obj


def normalize_case_job(
    *,
    sheet_name: str,
    case: Dict[str, Any],
    model: str,
    ...
    normalized_substeps = call_deepseek_case(
        ...
        case_obj=build_case_object(case),
    )
    ...
    return sheet_name, case_id, normalized_substeps


def fallback_normalize_case(case: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for step in case.get("steps", []):
        ...
        action_substeps = clean_text_list(step.get("action_substeps", [])) if isinstance(step.get("action_substeps"), list) else []
        expected_checkpoints = clean_text_list(step.get("expected_checkpoints", [])) if isinstance(step.get("expected_checkpoints"), list) else []
            ...
            fallback_steps.append(
                {
                    "step_no": step_no,
                    "sub_step_no": idx + 1,
                    "normalized_action": canonicalize_action_text(normalized_action),
                    "normalized_expected_result": canonicalize_expected_text(normalized_expected),
                    "keywords": [],
                }
            )
...
def main() -> None:
    ...
    parser.add_argument("--max-workers", type=int, default=8, help="Maximum concurrent case normalization workers.")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_case = {
            executor.submit(
                ...
                base_url=base_url,
                skill_text=skill_text,
            ): (sheet_name, case.get("case_id"))
            for sheet_name, case in jobs
        }

        completed = 0
        for future in as_completed(future_to_case):
            sheet_name, case_id = future_to_case[future]
            try:
                result_sheet, result_case_id, normalized_substeps = future.result()
            except Exception as e:
                print(f"Failed to normalize case {case_id}: {e}")
                # Fallback to deterministic normalization to avoid dropping full case.
                fallback_case = None
                for s_name, case in jobs:
                    if s_name == sheet_name and case.get("case_id") == case_id:
                        fallback_case = case
                        break
                if fallback_case is None:
                    continue
                normalized_payload[sheet_name][case_id] = fallback_normalize_case(fallback_case)
                completed += 1
                print(f"Fallback normalized case: {case_id} ({completed}/{total_jobs})")
                continue

            normalized_payload[result_sheet][result_case_id] = normalized_substeps
            completed += 1
            print(f"Normalized case: {result_case_id} ({completed}/{total_jobs})")
